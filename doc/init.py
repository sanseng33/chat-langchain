...
def extract_relevant_logs(folder_path):
    ...
    keywords = ["[callGPT] gpt调用结果", "background\":"]
    # keywords = ["[SituationModelWork] ai response"]
    keywordsResp = ["[callGPT] 参数前置处理","project_background"]
    results = []
    resultsResp = []
            ...
            if file.endswith(".txt"):
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        ...
                        if all(keyword in line for keyword in keywordsResp):
                            print(line)
                            results = fixRequest(line, results)
...
if __name__ == "__main__":
    ...
    csv_file_name = 'D://data//background.csv'
